"""
Scenario2 : 300×300 resolution + capped class weights + label smoothing
"""
...
def parse_args():
    ...
    p.add_argument("--model", type=str, default="dr_run5_model.keras", help="Model save path (.keras recommended)")
    p.add_argument("--run-tag", type=str, default="dr_run5", help="Run tag for evaluate.py folders")
    p.add_argument("--eval-after", action="store_true", help="Run evaluate.py after training")

    p.add_argument("--img", type=int, default=300, help="Image size (img x img) [Run5 default: 300]")
    p.add_argument("--batch", type=int, default=16, help="Batch size (300px uses more VRAM; default 16)")
    ...
    p.add_argument("--ft-lr", type=float, default=5e-6)

    p.add_argument("--unfreeze", type=int, default=120, help="Unfreeze last N layers in EfficientNet during fine-tune")
    p.add_argument("--label-smoothing", type=float, default=0.08, help="Label smoothing (reduces overconfident errors)")

    p.add_argument("--cw-max", type=float, default=2.0, help="Cap maximum class weight (Run5 default 2.0)")
    p.add_argument("--cw-min", type=float, default=0.6, help="Floor minimum class weight (Run5 default 0.6)")
    return p.parse_args()
...
class DRRun5:
    ...
    def compute_capped_class_weights(self, cw_min: float, cw_max: float):
        """
        Compute balanced weights, then cap to avoid Severe dominating training.
        """
        ...
        for k in class_weight:
            class_weight[k] = max(cw_min, min(cw_max, class_weight[k]))

        inv = {v: k for k, v in self.train_gen.class_indices.items()}
        print("Class weights (balanced then capped):")
        for cid in sorted(class_weight.keys()):
            print(f"  {cid} ({inv[cid]}): {class_weight[cid]:.4f}")
        print()
        return class_weight

    def build_model(self, lr: float, label_smoothing: float):
        ...
        # Label smoothing reduces overconfident mistakes (helps Moderate vs Severe confusion)
    ...
    def train_head(self, epochs: int, lr: float, class_weight: dict, model_path: str, label_smoothing: float):
    ...
    def fine_tune(self, epochs: int, ft_lr: float, unfreeze_last_n: int, class_weight: dict, model_path: str, label_smoothing: float):
...
def main():
    ...
    runner = DRRun5(args.data, args.img, args.batch, args.seed)
    ...
    class_weight = runner.compute_capped_class_weights(cw_min=args.cw_min, cw_max=args.cw_max)

    runner.build_model(lr=args.head_lr, label_smoothing=args.label_smoothing)
