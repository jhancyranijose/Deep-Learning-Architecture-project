"""
Scenario4 : 380×380 resolution + focal loss + stronger fine-tuning
"""
...
def parse_args():
    ...
    p.add_argument("--model", type=str, default="dr_run7_model.keras")
    p.add_argument("--run-tag", type=str, default="dr_run7")
    p.add_argument("--eval-after", action="store_true")

    p.add_argument("--img", type=int, default=380)
    p.add_argument("--batch", type=int, default=8, help="380px uses more VRAM. Try 8/6/4.")
    ...
    p.add_argument("--use-class-weights", action="store_true", help="Enable capped class weights (recommended)")
    p.add_argument("--cw-min", type=float, default=0.7)
    p.add_argument("--cw-max", type=float, default=1.8)

    p.add_argument("--mixed-precision", action="store_true", help="May help speed/VRAM on supported GPUs")
    return p.parse_args()
...
class CategoricalFocalLoss(keras.losses.Loss):
    ...
    def get_config(self):
        ...
        cfg.update({"gamma": self.gamma, "alpha": self.alpha, "from_logits": self.from_logits})
        return cfg
...
class DRRun7:
    ...
    def build_model(self, lr: float, gamma: float, alpha: float):
        print("Building model (EfficientNetB4 + GAP head + focal loss)...")
        ...
        loss_fn = CategoricalFocalLoss(gamma=gamma, alpha=alpha, from_logits=False)
    ...
    def train_head(self, epochs: int, lr: float, class_weight: dict, model_path: str, gamma: float, alpha: float):
    ...
    def fine_tune(self, epochs: int, ft_lr: float, unfreeze_last_n: int, class_weight: dict, model_path: str, gamma: float, alpha: float):
...
def main():
    ...
    runner = DRRun7(args.data, args.img, args.batch, args.seed)
